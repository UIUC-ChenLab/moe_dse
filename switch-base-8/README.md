This step generates expert token distribution for a given MoE, for DSE and validation. The benchmark is hugging face switch transformer base https://huggingface.co/google/switch-base-8. Install it on the test machine. Detailed steps are following:

1. Replace the following file from the installed hugging face with the file with the same name in this directory.
...
3. uncomment "part 1, part 2, part 3" in the `modeling_switch_transformers.py` for different purposes. Refer to the comment in the file for detail.
